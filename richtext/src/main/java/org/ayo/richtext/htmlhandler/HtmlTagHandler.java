...
public class HtmlTagHandler {
    ...
    public static Spanned fromHtml(String source, Html.ImageGetter getter, TagHandler tagHandler){
        if(source != null && !source.startsWith("<html>")){
    ...
    public abstract static class AbstractTagHandler implements TagHandler{
        ...
        @Override
        final public void handleTag(boolean open, String tag, Editable output, Attributes attrs) {
            if(!isForTag(tag)) return;
            if(open){
                this.tag = tag;
                this.attrs = attrs;
            }else{
                this.output = output;
                output.setSpan(parseAttrtoSpan(this.tag, this.attrs), 0, output.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }

        protected abstract Object parseAttrtoSpan(String tag, Attributes attrs);
        protected abstract boolean isForTag(String tag);
    }
